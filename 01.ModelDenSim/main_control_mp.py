...
def calc_cc(mtz_pair):
    print(mtz_pair)

    mtz1_name,mtz2_name=mtz_pair
...
for index,mtz1 in enumerate(mtzs):
    for index2,mtz2 in enumerate(mtzs[index+1:]):
        proc_list.append((mtz1,mtz2))
        
print "making %05d files"%len(proc_list)
result=easy_mp.pool_map(fixed_func=lambda n: calc_cc(n), args=proc_list, processes=56)
