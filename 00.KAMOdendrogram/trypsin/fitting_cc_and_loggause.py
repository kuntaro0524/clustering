...
def log_gaussian(x, alpha, loc, scale):
    # lognorm
    from scipy.stats import lognorm
    rtn_value = lognorm.pdf(1-x, alpha, loc, scale) 
    #rtn_value = skewnorm.pdf(x, alpha, loc, scale)
    return rtn_value

# 初期値の設定と最小二乗法の計算
initial_guess = [0.93, 0.003, 0.001]
popt, pcov = curve_fit(log_gaussian, bin_centers, hist, p0=initial_guess)
alpha_fit, loc_fit, scale_fit = popt

# Histogram drawing
plt.hist(ccdata, bins=n_bins, color='green', alpha=0.5,density=False)
...
xp = np.arange(0.0, 1.0, 0.0001)
# plotの縦軸は右側の軸を指定
#plt.twinx()
plt.plot(xp, log_gaussian(xp, *popt), 'r-', label='Fitted Skew Gaussian')
plt.xlim(0.8,1.0)
# y軸は左と右で同じ
#plt.ylim(0, 100)
