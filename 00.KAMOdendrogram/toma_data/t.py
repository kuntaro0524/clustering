...
def fit(cc_df, cc_threshold = 0.8):
    ...
    # 初期値の設定と最小二乗法の計算
    initial_guess = [0, np.mean(ccdata), np.std(ccdata), 1]
    popt, pcov = curve_fit(beta_pdf, bin_centers, hist, p0=initial_guess)
    alpha_fit, beta_fit, loc_fit, scale_fit = popt
    ...
    plt.plot(bin_centers, beta_pdf(bin_centers, *popt), 'r-', label='Fitted Skew Gaussian')
